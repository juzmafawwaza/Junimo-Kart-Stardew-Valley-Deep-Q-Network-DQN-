...
def evaluate_model(
    ...
    grounded_progress_bonus_coef: float,
    unnecessary_jump_penalty: float,
    non_gap_airborne_penalty: float,
    gap_tip_technique_reward: float,
    takeoff_tip_target_distance: float,
    takeoff_tip_tolerance: float,
    landing_tip_target_depth: float,
    landing_tip_tolerance: float,
    takeoff_target_width_coef: float,
    takeoff_target_uphill_coef: float,
    takeoff_target_downhill_coef: float,
    takeoff_target_min_distance: float,
    takeoff_target_max_distance: float,
    takeoff_dynamic_tolerance: float,
    gap_inaction_margin: float,
    gap_inaction_penalty: float,
    ...
    env = JunimoKartEnv(
        ...
        grounded_progress_bonus_coef=grounded_progress_bonus_coef,
        unnecessary_jump_penalty=unnecessary_jump_penalty,
        non_gap_airborne_penalty=non_gap_airborne_penalty,
        gap_tip_technique_reward=gap_tip_technique_reward,
        takeoff_tip_target_distance=takeoff_tip_target_distance,
        takeoff_tip_tolerance=takeoff_tip_tolerance,
        landing_tip_target_depth=landing_tip_target_depth,
        landing_tip_tolerance=landing_tip_tolerance,
        takeoff_target_width_coef=takeoff_target_width_coef,
        takeoff_target_uphill_coef=takeoff_target_uphill_coef,
        takeoff_target_downhill_coef=takeoff_target_downhill_coef,
        takeoff_target_min_distance=takeoff_target_min_distance,
        takeoff_target_max_distance=takeoff_target_max_distance,
        takeoff_dynamic_tolerance=takeoff_dynamic_tolerance,
        gap_inaction_margin=gap_inaction_margin,
        gap_inaction_penalty=gap_inaction_penalty,
    )
    ...
    row = {
        ...
        "mean_grounded_progress_bonus_total": telemetry_totals["grounded_progress_bonus_total"] / episodes,
        "mean_unnecessary_jump_events": telemetry_totals["unnecessary_jump_events"] / episodes,
        "mean_unnecessary_jump_penalty_total": telemetry_totals["unnecessary_jump_penalty_total"] / episodes,
        "mean_non_gap_airborne_steps": telemetry_totals["non_gap_airborne_steps"] / episodes,
        "mean_non_gap_airborne_penalty_total": telemetry_totals["non_gap_airborne_penalty_total"] / episodes,
        "mean_takeoff_tip_quality": telemetry_totals["takeoff_tip_quality_total"] / telemetry_totals["gap_takeoff_events"] if telemetry_totals["gap_takeoff_events"] else 0.0,
        "mean_successful_takeoff_tip_quality": telemetry_totals["successful_takeoff_tip_quality_total"] / telemetry_totals["edge_qualified_landings"] if telemetry_totals["edge_qualified_landings"] else 0.0,
        "mean_landing_tip_quality": telemetry_totals["landing_tip_quality_total"] / telemetry_totals["edge_qualified_landings"] if telemetry_totals["edge_qualified_landings"] else 0.0,
        "mean_edge_technique_reward_total": telemetry_totals["edge_technique_reward_total"] / episodes,
        "mean_takeoff_target_distance": telemetry_totals["takeoff_target_distance_total"] / telemetry_totals["gap_takeoff_events"] if telemetry_totals["gap_takeoff_events"] else 0.0,
        "mean_successful_takeoff_distance": telemetry_totals["successful_takeoff_tip_distance_total"] / telemetry_totals["edge_qualified_landings"] if telemetry_totals["edge_qualified_landings"] else 0.0,
        "mean_successful_takeoff_target_distance": telemetry_totals["successful_takeoff_target_distance_total"] / telemetry_totals["edge_qualified_landings"] if telemetry_totals["edge_qualified_landings"] else 0.0,
        "mean_gap_inaction_steps": telemetry_totals["gap_inaction_steps"] / episodes,
        "mean_gap_inaction_penalty_total": telemetry_totals["gap_inaction_penalty_total"] / episodes,
    }
    for idx in range(4):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate one or more Junimo Kart PPO checkpoints deterministically.")
    parser.add_argument("models", nargs="+", help="PPO model checkpoint .zip files.")
    ...
    parser.add_argument("--reward-version", default="legacy", choices=["legacy", "shaped_v1", "shaped_v2", "shaped_v3", "shaped_v4", "shaped_v5", "shaped_v6", "shaped_v7", "shaped_v8", "shaped_v9"], help="Reward function to use during evaluation.")
    ...
    parser.add_argument("--grounded-progress-bonus-coef", type=float, default=0.005)
    parser.add_argument("--unnecessary-jump-penalty", type=float, default=0.15)
    parser.add_argument("--non-gap-airborne-penalty", type=float, default=0.01)
    parser.add_argument("--gap-tip-technique-reward", type=float, default=1.5)
    parser.add_argument("--takeoff-tip-target-distance", type=float, default=12.0)
    parser.add_argument("--takeoff-tip-tolerance", type=float, default=48.0)
    parser.add_argument("--landing-tip-target-depth", type=float, default=16.0)
    parser.add_argument("--landing-tip-tolerance", type=float, default=64.0)
    parser.add_argument("--takeoff-target-width-coef", type=float, default=0.65)
    parser.add_argument("--takeoff-target-uphill-coef", type=float, default=0.25)
    parser.add_argument("--takeoff-target-downhill-coef", type=float, default=0.10)
    parser.add_argument("--takeoff-target-min-distance", type=float, default=32.0)
    parser.add_argument("--takeoff-target-max-distance", type=float, default=112.0)
    parser.add_argument("--takeoff-dynamic-tolerance", type=float, default=64.0)
    parser.add_argument("--gap-inaction-margin", type=float, default=24.0)
    parser.add_argument("--gap-inaction-penalty", type=float, default=0.05)
    ...
    parser.add_argument("--out", default="logs/ppo/evaluation.csv")
    ...
    for pattern in args.models:
        ...
        if any(char in pattern for char in "*?[]"):
            raise SystemExit(
                f"No PPO checkpoint matched pattern: {pattern}\n"
    ...
    for model in model_paths:
        try:
            print(f"Evaluating PPO checkpoint: {model}", flush=True)
            rows.append(
                evaluate_model(
                    model,
                    args.episodes,
                    ...
                    args.grounded_progress_bonus_coef,
                    args.unnecessary_jump_penalty,
                    args.non_gap_airborne_penalty,
                    args.gap_tip_technique_reward,
                    args.takeoff_tip_target_distance,
                    args.takeoff_tip_tolerance,
                    args.landing_tip_target_depth,
                    args.landing_tip_tolerance,
                    args.takeoff_target_width_coef,
                    args.takeoff_target_uphill_coef,
                    args.takeoff_target_downhill_coef,
                    args.takeoff_target_min_distance,
                    args.takeoff_target_max_distance,
                    args.takeoff_dynamic_tolerance,
                    args.gap_inaction_margin,
                    args.gap_inaction_penalty,
    ...
    print(f"Saved PPO evaluation CSV to: {out}")
